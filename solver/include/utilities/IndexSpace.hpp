...
class IndexSpace {
 public:
  ...
  Index increment_joint_index(Index joint_index, std::size_t local_order, Index local_index) const
  {
    return joint_index + local_index * stepsize_[local_order];
  }

  Index joint_index(const std::vector<Index>& indices) const {
    Index joint_index = 0;
    for (vec_size_t i = 0; i < stepsize_.size(); ++i)
      joint_index = increment_joint_index(joint_index, i, indices[i]);
